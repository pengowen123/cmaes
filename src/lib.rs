...
//! objective function, and performs well on high-dimension, non-convex, ill-conditioned, and/or
//! noisy problems.
//!
//! // TODO: example
//!
...
pub enum TerminationReason {
    ...
    /// `step_size * p_c` is smaller than `tol_x` in every coordinate (meaning that the mean has not
    /// moved much in the past few generations).
    TolX,
...
impl<F: Fn(&DVector<f64>) -> f64> CMAESState<F> {
    ...
    pub fn new(options: CMAESOptions<F>) -> Result<Self, InvalidOptionsError> {
        ...
        if options.dimensions == 0 {
            return Err(InvalidOptionsError::ZeroDimensions);
        }
        ...
        if options.population_size < 4 {
            return Err(InvalidOptionsError::SmallPopulationSize);
        }
        ...
        let cmu = (1.0 - c1)
            .min(a_cov * (mu_eff - 2.0 + 1.0 / mu_eff)
        ...
        let eigen = normalized_symmetric_eigen(cov.clone());
        let cov_eigenvectors = eigen.eigenvectors;
        let cov_eigenvalues = eigen.eigenvalues;
    ...
    pub fn next(&mut self) -> Option<TerminationData> {
        ...
        let old_max_standard_deviation =
            *sigma * cov_eigenvalues.diagonal().iter().max_by(|a, b| partial_cmp(*a, *b)).unwrap();
    ...
    fn check_termination_criteria(
        ...
        if self.best_function_values.len() >= past_generations_a {
            let max = self.best_function_values
                .iter()
                .take(past_generations_a)
                .max_by(|a, b| a.partial_cmp(b).unwrap_or(Ordering::Equal))
                .unwrap();

            let min = self.best_function_values
                .iter()
                .take(past_generations_a)
        ...
        if (0..dim).all(|i| *sigma * cov[(i, i)] < tol_x)
            && path_c.iter().all(|x| *sigma * *x < tol_x) {

            return Some(TerminationReason::TolX);
        }

        // Check TerminationReason::ConditionCov
        let diag = cov_eigenvalues.diagonal();
        let max = diag.iter().max_by(|a, b| partial_cmp(*a, *b)).unwrap();
        let min = diag.iter().min_by(|a, b| partial_cmp(*a, *b)).unwrap();
        let cond = (max / min).abs();

        if !cond.is_normal() || cond > 10e4 {
        ...
        let no_effect_axis_check =
            0.1 * *sigma
                * cov_eigenvalues[(index_to_check, index_to_check)]
                * cov_eigenvectors.column(index_to_check);

        if no_effect_axis_check.iter().all(|x| x.abs() < EPSILON) {
            return Some(TerminationReason::NoEffectAxis);
        }

        // Check TerminationReason::NoEffectCoord
        if (0..dim).any(|i| 0.2 * *sigma * cov_eigenvalues[(i, i)].abs() < EPSILON) {
        ...
        if self.best_function_values.len() >= past_generations_b
            ...
            let did_values_improve = |values: &VecDeque<f64>| {
                ...
                statistical::median(&first_best_values)
                    <= statistical::median(&last_best_values) - EPSILON
            };
        ...
        let new_max_standard_deviation =
            *sigma * cov_eigenvalues.diagonal().iter().max_by(|a, b| partial_cmp(*a, *b)).unwrap();

        if (new_max_standard_deviation / old_max_standard_deviation) > 10e4 {
    ...
    pub fn get_current_best_individual(&self) -> Option<(&DVector<f64>, f64)> {
        self.best_function_values.front().map(|x| {
            (self.current_best_individual.as_ref().unwrap(), *x)
        })
    }
}
...
// Computes the eigendecomposition of a matrix, normalizes the eigenvectors, and puts the
// eigenvalues into a diagonal matrix
fn normalized_symmetric_eigen(matrix: DMatrix<f64>) -> NormalizedEigen {
    ...
    NormalizedEigen {
        ...
        eigenvalues: DMatrix::from_diagonal(&eigen.eigenvalues),
    }
}
...
mod tests {
    ...
    use crate::{CMAESOptions, Weights};

    fn dummy_function(_: &DVector<f64>) -> f64 { 0.0 }
    ...
    fn test_weights_negative() {
        for n in 1..10 {
            ...
            assert!(
                cmaes_state.parameters.weights
                    .iter()
